...
def create_notification_like_thread(sender, instance, **kwargs):
    ...
    if not instance.thread.user == instance.user:
        Notification.objects.create(
            ...
            type="like_thread",
            content=f"{instance.user.username} likes your tale: {instance.thread.content[:20]}...",
...
def create_notification_repost_thread(sender, instance, **kwargs):
    ...
    if not instance.user == instance.thread.user:
        Notification.objects.create(
            user=instance.thread.user,
            type="repost_thread",
            content=f"{instance.user.username} reposted your tale: {instance.thread.content[:20]}...",
            actioner=instance.user,
        )
    ...
    if instance.parent_comment:
        ...
        if not instance.thread.user == instance.user:
            Notification.objects.create(
                user=instance.thread.user,
                type="comment",
                content=f"{instance.user.username} reply to your tale: {instance.thread.content[:20]}...",
                actioner=instance.user,
            )
